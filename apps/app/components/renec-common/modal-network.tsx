...
const ModalNetworkPage = ({ isOpen, onClose }: any) => {
  ...
  return (
    <div className="fixed top-0 h-full w-full flex flex-col justify-center items-center bg-black/30 backdrop-blur-sm z-[3]">
      <div className="relative flex flex-col rounded-2xl h-full w-full max-w-[500px] max-h-[250px] bg-[#0d1624]  p-5">
                  ...
                  .map((data, index) => {
                    return (
                      <button
                        ...
                        key={index}
                        className="flex flex-row items-center gap-2 bg-[#636b79]/20 rounded-full px-4 py-2 hover:brightness-125 duration-300  active:scale-95"
                      >
                        <Image
                          src={data.imgUrl}
                          alt={"refresh"}
                          ...
                          width={25}
                        />
                        <p className="mobile-title sm:tablet-title lg:web-title text-[#D3DAE6]">
                          {data.shortname}
                        </p>
                      </button>
                    );
                      ...
                      {chains
                        ...
                        .map((x) => (
                          <button
                            ...
                            className={`flex flex-row items-center gap-2 bg-[#636b79]/20  rounded-full ${
                              chain?.id === x.id
                                ? "cursor-not-allowed"
        ...
        <div className="absolute top-0 left-0 border-[1px] border-[#363e4d] h-full w-full z-[1] rounded-2xl faded-bottom" />
      </div>
    </div>
  );
};
