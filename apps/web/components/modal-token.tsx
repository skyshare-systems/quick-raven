...
const ModalTokenPage = ({ onClose, isOpen, initNetwork }: any) => {
  const hide = "hidden";
  const show = "auto";
  const [searchTokenPolygon, setSearchTokenPolygon] = useState("");
          ...
          <div className="relative flex flex-col p-5 z-[2] w-full gap-5 ">
            ...
            <div className="flex flex-row justify-between gap-5 px-5  border-[1px] border-[#424242] rounded-xl ">
              ...
              <input
                ...
                onChange={(e) => setSearchTokenPolygon(e.target.value)}
              />
            </div>
            {/* white list token  */}
            ...
            <div className="flex flex-col justify-center w-full gap-5">
              {tokenpolygon
                .filter((dataFilter) => {
                  return searchTokenPolygon.toLowerCase() === "" ||
                    searchTokenPolygon.toUpperCase() === ""
                    ? dataFilter
                    : dataFilter.tokenName
                        .toLowerCase()
                        .includes(searchTokenPolygon) ||
                        dataFilter.tokenName
                          .toUpperCase()
                          .includes(searchTokenPolygon);
                })
